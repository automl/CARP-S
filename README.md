...
It also directly checks for missing runs which you can rerun via `runcommands_missing.sh`.
The file `runstatus.csv` contains the run status of all found runs.

### Plotting
Creating a report works with
```bash
python -m carps.analysis.generate_report <rundir>
```
and outputs figures and a report into `reports/<timestamp>`.
