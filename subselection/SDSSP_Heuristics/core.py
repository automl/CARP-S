...
setup_logging()
logger = get_logger("Subselect:core")
...
def subselect(points_df: pd.DataFrame, k: int, n_reps: int = 5000, executable: str = "./a.out") -> tuple[pd.DataFrame, dict]:
    ...
    Args:
        points_df (pd.DataFrame): DataFrame containing the points to be subselected. Index can be the task id and the
            columns can be different optimizers. Values must be in unit cube.
    ...
    pointfile = Path(pointfile).resolve()
    ...
    command = f"export SHIFT_TRIES={n_reps}; {executable} {pointfile!s} {dimension} {n_points} {k} {outfile!s}"
    logger.info(command)
    result = subprocess.run(["bash", "-c", command], capture_output=True, text=True, check=False)
    has_errored = result.returncode != 0
    if has_errored:
        logger.info(f"stderr from executable: {result.stderr}")
        raise ValueError(f"Subselection failed with error code {result.returncode}")
    logger.info(f"stdout from executable: {result.stdout}")
    logger.info(f"stderr from executable: {result.stderr[:500]}")
...
def select_sets_split(
    ...
    Args:
        points_df (pd.DataFrame): DataFrame containing the points. Values must be in unit cube.
