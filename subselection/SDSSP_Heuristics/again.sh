...
command_function() {
    ...
    # Note: this command `while IFS= read -r line; do echo "$(date '+%Y-%m-%d %H:%M:%S') $line"; done` is used to prepend the date to each line of the output
    # if you have sudo access, you can use `ts` command instead
    SHIFT_TRIES=${SHIFT_TRIES:-5000} ${executable} subset_complement_${value}.txt ${dimension} $(expr ${num_points} - ${value}) ${value} subset_complement_subset_${value}.txt 2>&1 | while IFS= read -r line; do echo "$(date '+%Y-%m-%d %H:%M:%S') $line"; done > log_subset_complement_${value}.txt
