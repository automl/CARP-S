...
def gather_trajectory(x: pd.DataFrame) -> pd.DataFrame:
    ...
    for n_trials, gdf in x.groupby("n_trials"):
        cost_inc = (
            gdf["trial_value__cost_raw"].apply(eval).apply(
                lambda x: np.array([np.array(c) for c in x])).to_numpy()
        )
...
def calculate_hypervolume(rundir: str) -> None:
    ...
    trajectory_df = (
        trajectory_df.groupby(by=["scenario", "problem_id"]).apply(add_reference_point).reset_index(
            drop=True)
    )
    trajectory_df = trajectory_df.groupby(by=[*run_id, "n_trials"]).apply(calc_hv).reset_index(
        drop=True)
...
def load_trajectory(rundir: str) -> pd.DataFrame:
    ...
    if not fn.is_file():
        raise ValueError(
            f"Cannot find {fn}. Did you run `python -m carps.analysis.calc_hypervolume {rundir}`?")
    df = pd.read_parquet(fn)
