...
def plot_final_performance_violinplot(
        df: pd.DataFrame, x="n_trials_norm", y="trial_value__cost_inc_norm", hue="optimizer_id", 
        budget_var: str = "n_trials_norm", max_budget: float = 1,
        figure_filename: str = "figures/final_performance_boxplot.pdf", figsize: tuple[int,int]=(6,4), **violinplot_kwargs
    ) -> tuple[plt.Figure, matplotlib.axes.Axes]:
    ...
    fig = plt.figure(figsize=figsize, dpi=300)
    ax = fig.add_subplot(111)
    ax = sns.violinplot(data=df[np.isclose(df[budget_var], max_budget)], y=y, x=x, hue=hue, palette=palette, ax=ax, cut=0, **violinplot_kwargs)
    savefig(fig, figure_filename)
    return fig, ax
