...
def filter_task_info(logs: pd.DataFrame,
                     keep_task_columns: list[str] | None = None) -> pd.DataFrame:
...
def load_set(paths: list[str], set_id: str = "unknown") -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df_cfg = pd.concat([pd.read_parquet(Path(p) / "logs_cfg.parquet") for p in paths]).reset_index(
        drop=True)
    df["set"] = set_id
    return df, df_cfg
...
def process_logs(logs: pd.DataFrame, keep_task_columns: list[str] | None = None) -> pd.DataFrame:
    ...
    if "trial_value__additional_info" in logs:
        ...
        if len(ids_mo) > 0:
            logs[ids_mo]["trial_value__cost_raw"] = logs[ids_mo][
                "trial_value__additional_info"].apply(
                lambda x: x["cost"]
            )
