...
def convert_mixed_types_to_str(logs: pd.DataFrame, logger=None) -> pd.DataFrame:
    ...
    if logger:
        logger.debug(
            f"Goodybe all mixed data, ruthlessly converting {mixed_type_columns} to str...")
...
def normalize_logs(logs: pd.DataFrame) -> pd.DataFrame:
    ...
    logs["trial_value__cost_norm"] = logs.groupby("problem_id")["trial_value__cost"].transform(
        normalize)
    logger.info("Calc normalized incumbent cost...")
    logs["trial_value__cost_inc_norm"] = logs.groupby(by=["problem_id", "optimizer_id", "seed"])[
        "trial_value__cost_norm"].transform("cummin")
    if "time" not in logs:
        logs["time"] = 0
...
def get_interpolated_performance_df(logs: pd.DataFrame, n_points: int = 20,
                                    x_column: str = "n_trials_norm",
                                    interpolation_columns: list[str] | None = None) -> pd.DataFrame:
    ...
    if interpolation_columns is None:
        interpolation_columns = ["trial_value__cost", "trial_value__cost_norm",
...
def filter_only_final_performance(df: pd.DataFrame, budget_var: str = "n_trials_norm",
                                  max_budget: float = 1, soft: bool = True) -> pd.DataFrame:
    if not soft:
        df = df[np.isclose(df[budget_var], max_budget)]
    else:
        df = df[df.groupby(["optimizer_id", "problem_id", "seed"])[budget_var].transform(
            lambda x: x == x.max())]
    return df


def plot_performance_over_time(df: pd.DataFrame, x="n_trials_norm", y="cost_inc_norm",
                               hue="optimizer_id",
                               figure_filename: str = "figures/performance_over_time.pdf",
                               figsize: tuple[int, int] = (6, 4), show_legend: bool = True,
                               **lineplot_kwargs
...
def get_df_crit(df: pd.DataFrame, budget_var: str = "n_trials_norm", max_budget: float = 1,
                soft: bool = True, perf_col: str = "trial_value__cost_inc",
                remove_nan: bool = True) -> pd.DataFrame:
    df = filter_only_final_performance(df=df, budget_var=budget_var, max_budget=max_budget,
                                       soft=soft)
