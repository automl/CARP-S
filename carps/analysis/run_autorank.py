...
def custom_latex_table(result, *, decimal_places=3, label=None, only_tabular: bool = True) -> str:
    ...
    format_string = "[{0[ci_lower]:." + str(decimal_places) + "f}, {0[ci_upper]:." + str(
        decimal_places) + "f}]"
    ...
    float_format = lambda x: ("{:0." + str(decimal_places) + "f}").format(x) if not np.isnan(
        x) else "-"
...
def calc_critical_difference(
        df: pd.DataFrame, identifier: str | None = None, figsize=(12, 8),
...
def _custom_cd_diagram(result, reverse, ax, width):
    ...
    for i in range(math.ceil(len(sorted_ranks) / 2)):
        ...
        plot_line(
            [(rankpos(sorted_ranks[i]), cline), (rankpos(sorted_ranks[i]), chei),
             (textspace - 0.1, chei)],
            linewidth=0.7,
        )
