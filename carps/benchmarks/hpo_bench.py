...
class HPOBenchProblem(Problem):
    ...
    def __init__(
        ...
        self._problem = (
            problem
            if problem
            else get_hpobench_problem(task_id=task_id, model=model, seed=seed,
    ...
    def _evaluate(self, trial_info: TrialInfo) -> TrialValue:
        ...
        if trial_info.budget is not None and self.budget_type is not None:
            budget_value = float(trial_info.budget) if self.budget_type == "subsample" else round(
                trial_info.budget)
...
def get_hpobench_problem(model: str, task_id: int, seed: int,
                         budget_type: str | None = None) -> Any:
    ...
    if model == "lr":
        problem = TabularBenchmark(model="lr",
                                   **common_args) if budget_type is None else LRBenchmark(
            **common_args)
    elif model == "nn":
        problem = TabularBenchmark(model="nn",
                                   **common_args) if budget_type is None else NNBenchmark(
            **common_args)
    elif model == "rf":
        problem = (
            TabularBenchmark(model="rf",
                             **common_args) if budget_type is None else RandomForestBenchmark(
                **common_args)
        )
    elif model == "svm":
        problem = TabularBenchmark(model="svm",
                                   **common_args) if budget_type is None else SVMBenchmark(
            **common_args)
    elif model == "xgboost":
        problem = (
            TabularBenchmark(model="xgb",
                             **common_args) if budget_type is None else XGBoostBenchmark(
                **common_args)
        )
    else:
