...
```
The generated log files are then located in `<rundir>`. `logs.csv` contains the trial info and values while 
`logs_cfg.parquet` contains the experiment configuration.
