...
class DatabaseConnectorMySQL(DatabaseConnectorMYSQL):
    ...
    def find_experiment_id(self, parameters: dict[str, Any]) -> int:
        ...
        if len(ids) > 1:
            warnings.warn(
                "Something is weird. Got multiple experiment ids for the same parameters. " f"{ids} from query {query}."
            )
        return ids[0]
    ...
    def create_table_if_not_existing(self) -> None:
        ...
        if self._table_exists(cursor):
            ...
            self._create_table(
                ...
                table_type="logtable",
                logatableaddition=self.logtableaddition,
            )
    ...
    def _create_table(
        ...
        logatableaddition: str | None = None,
    ):
